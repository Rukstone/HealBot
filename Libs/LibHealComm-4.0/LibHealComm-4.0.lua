...
elseif (playerClass == "SUNCLERIC") then
	LoadClassData = function()
		-- Hot data
		local SunCleric_Sunshine = GetSpellInfo(502412) or "Sunshine";
		hotData[SunCleric_Sunshine] = {
			coeff = 1,
			interval = 3,
			ticks = 5,
			levels = { 8, 14, 20, 26, 32, 38, 44, 50, 56, 60, 65, 70, 75, 80 },
		...
		local SunCleric_Illumination = GetSpellInfo(502423) or "Illumination";
		spellData[SunCleric_Illumination] = {
			coeff = 3 / 3.5,
			levels = { 40, 46, 52, 58, 60, 63, 68, 73, 78 },
			increase = { 204, 197, 184, 165, 162, 142, 111, 92, 30 },
			averages = { avg(899, 1013), avg(1149, 1289), avg(1437, 1609), avg(1798, 2006), avg(1966, 2194),
		...
		-- Prayer of Healing
		local SunCleric_Prayer_Of_The_Sands = GetSpellInfo(502432) or "Prayer of the Sands";
		spellData[SunCleric_Prayer_Of_The_Sands] = {
			coeff = 0.2798,
			levels = { 30, 40, 50, 60, 60, 68, 76 },
			increase = { 65, 64, 60, 48, 50, 33, 18 },
			averages = { avg(301, 321), avg(444, 472), avg(657, 695), avg(939, 991), avg(997, 1053), avg(1246, 1316),
		...
		-- Flash Heal
		local SunCleric_Shine = GetSpellInfo(502449) or "Shine";
		spellData[SunCleric_Shine] = {
			coeff = 1.5 / 3.5,
			levels = { 20, 26, 32, 38, 44, 52, 58, 61, 67, 73, 79 },
			increase = { 114, 118, 120, 117, 118, 111, 100, 89, 67, 56, 9 },
			averages = { avg(193, 237), avg(258, 314), avg(327, 393), avg(400, 478), avg(518, 616), avg(644, 764),
		...
		-- Check for beacon when figuring out who to heal
		GetHealTargets = function(bitType, guid, healAmount, spellName, hasVariableTicks)
			if (spellName == SunCleric_Prayer_Of_The_Sands) then
				local targets = compressGUID[guid]
				local group = guidToGroup[guid]
		...
		CalculateHotHealing = function(guid, spellID)
			...
			if (spellName == SunCleric_Sunshine) then
				healModifier = healModifier + talentData[ImprovedRenew].current
				healModifier = healModifier + talentData[TwinDisciplines].current
				totalTicks = 5
				spellPower = spellPower *
		...
		CalculateHealing = function(guid, spellName, spellRank)
			...
			healModifier = healModifier + talentData[BlessedResilience].current
			healModifier = healModifier + talentData[SpiritualHealing].current
			if (spellName == SunCleric_Illumination) then
				spellPower = spellPower *
					((spellData[spellName].coeff * 1.88) * (1 + talentData[EmpoweredHealing].current))
			elseif (spellName == SunCleric_Shine) then
				spellPower = spellPower *
					((spellData[spellName].coeff * 1.88) * (1 + talentData[EmpoweredHealing].spent * 0.04))
			elseif (spellName == SunCleric_Prayer_Of_The_Sands) then
				healModifier = healModifier + talentData[DivineProvidence].current
				spellPower = spellPower * (spellData[spellName].coeff * 1.88)
			end
