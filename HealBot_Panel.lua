...
function HealBot_Panel_TestBarShow(index, button)
    ...
    if not HealBot_setTestCols then
        if (Healbot_Config_Skins.SetBarClassColour[Healbot_Config_Skins.Current_Skin] == 1) then
            if (Healbot_Config_Skins.SetBarCustomColour[Healbot_Config_Skins.Current_Skin] == 1) then
                if index == 1 then
                    HealBot_colIndex["hcr" .. index], HealBot_colIndex["hcg" .. index], HealBot_colIndex["hcb" .. index] =
                        HealBot_Action_ClassColour(HealBot_PlayerGUID, "player")
                else
                    HealBot_colIndex["hcr" .. index], HealBot_colIndex["hcg" .. index], HealBot_colIndex["hcb" .. index] =
                        HealBot_Panel_RandomClassColour()
                end
            ...
            HealBot_colIndex["hcr" .. index], HealBot_colIndex["hcg" .. index], HealBot_colIndex["hcb" .. index] = 0, 1,
                0
        end
        if HealBot_keepClass then
            HealBot_colIndex["hctr" .. index], HealBot_colIndex["hctg" .. index], HealBot_colIndex["hctb" .. index] =
                HealBot_colIndex["hcr" .. index], HealBot_colIndex["hcg" .. index], HealBot_colIndex["hcb" .. index]
        else
            if Healbot_Config_Skins.SetClassColourText[Healbot_Config_Skins.Current_Skin] == 1 then
                if index == 1 then
                ...
                HealBot_colIndex["hctr" .. index] = Healbot_Config_Skins.btextenabledcolr
                    [Healbot_Config_Skins.Current_Skin]
                HealBot_colIndex["hctg" .. index] = Healbot_Config_Skins.btextenabledcolg
                    [Healbot_Config_Skins.Current_Skin]
                HealBot_colIndex["hctb" .. index] = Healbot_Config_Skins.btextenabledcolb
                    [Healbot_Config_Skins.Current_Skin]
            end
        end
    ...
    bar:SetStatusBarColor(HealBot_colIndex["hcr" .. index], HealBot_colIndex["hcg" .. index],
        HealBot_colIndex["hcb" .. index], Healbot_Config_Skins.Barcola[Healbot_Config_Skins.Current_Skin]);
    bar.txt = _G[bar:GetName() .. "_text"];
    ...
    bar.txt:SetTextColor(HealBot_colIndex["hctr" .. index], HealBot_colIndex["hctg" .. index],
        HealBot_colIndex["hctb" .. index], Healbot_Config_Skins.btextenabledcola[Healbot_Config_Skins.Current_Skin]);
    bar:Show()
end

local HealBot_randomN = 1
local HealBot_randomClCol = {
    [1] = { [1] = 1.0, [2] = 0.49, [3] = 0.04, },
...
function HealBot_Panel_RandomClassColour()
    ...
    return HealBot_randomClCol[HealBot_randomN][1], HealBot_randomClCol[HealBot_randomN][2],
...
function HealBot_Panel_PanelChanged(showHeaders)
    ...
    if x then -- Crash Protection
        for j = 1, tonumber(HealBot_cpData["numMacros"]) do
            local u1n, u1u, u1g, u2n, u2u, u2g, u3n, u3u, u3g, u4n, u4u, u4g, u5n, u5u, u5g, u6n, u6u, u6g, u7n, u7u, u7g, u8n, u8u, u8g, u9n, u9u, u9g, u10n, u10u, u10g =
                string.split(":", HealBot_cpData["body" .. j])
        ...
        local _, c1, h1p, h1n, h2p, h2n, h3p, h3n, h4p, h4n, h5p, h5n, h6p, h6n, h7p, h7n, h8p, h8n, h9p, h9n, h10p, h10n, h11p, h11n, h12p, h12n =
            string.split(":", HealBot_cpData["body0"])
                        ...
                        elseif Healbot_Config_Skins.CombatProt[Healbot_Config_Skins.Current_Skin] == 1 and
                            ...
                            if not HealBot_TrackNames[xGUID] then
                                ...
                                if Healbot_Config_Skins.ExtraIncGroup[Healbot_Config_Skins.Current_Skin][subgroup] then
                                    ...
                                    elseif Healbot_Config_Skins.ExtraOrder[Healbot_Config_Skins.Current_Skin] == 2 then
                                        order[xUnit] = string.upper(HEALBOT_WARRIOR);
                                    elseif Healbot_Config_Skins.ExtraOrder[Healbot_Config_Skins.Current_Skin] == 3 then
                                        order[xUnit] = 8;
                                    elseif Healbot_Config_Skins.ExtraOrder[Healbot_Config_Skins.Current_Skin] == 4 then
            ...
            for j = 1, 41 do
                if not units[j] then
                    ...
                    if UnitExists(xUnit) then
                        ...
                        classEN = string.upper(HEALBOT_WARRIOR);
                        xGUID = xUnit
                    ...
                    if k < 5 then
                        ...
                        if xUnit then
                            if UnitGUID(xButton.unit) then
                                ...
                                HealBot_cpData["body" .. k] = HealBot_cpData["body" .. k] ..
                                    strsub(UnitName(xButton.unit), 1, 3) .. ":" .. xUnit .. ":" .. xGUID .. ":"
                            else
                                Delay_RecalcParty = 1
                            end
                        end
        ...
        table.foreach(HealBot_Action_HealButtons, function(x, xButton)
            ...
            if uName then
                ...
                if HeaderPos[i] then
                    ...
                    if not HealBot_BottomAnchors then
                        OffsetY = HealBot_Action_PositionButton(nil, OffsetX, OffsetY,
                            bwidth, bheight, header, curcol);
                    end
